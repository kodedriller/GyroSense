...
setInterval(() => {
  fetch("http://localhost:8000/")
    ...
    .then((axis) => {
      ...
      if (gameInProgress && accelData) {
        console.log(axis["accy"], axis["accx"]);
        const mouseDeltaX = -Math.minmax(mouseStartX + axis["accx"] * 2, 1000);
        const mouseDeltaY = -Math.minmax(mouseStartY - axis["accy"] * 2, 1000);
